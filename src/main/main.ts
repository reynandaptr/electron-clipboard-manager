...
const pushPendingClipboard = () => {
  if (pendingClipboard.length > 0) {
    pendingClipboard.forEach((e) => {
      mainWindow?.webContents.send('clipboard', e);
    });
    pendingClipboard = [];
  }
};

ipcMain.on('pending-clipboard', async (event, arg) => {
  console.log('sss', pendingClipboard);
  pushPendingClipboard();
});
